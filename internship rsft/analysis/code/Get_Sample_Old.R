...
Get_Sample_Old <- function(s1, s2, r1, r2, ps1, pr1, dfe_n, subject_n, budget, beta_n = 100, prior = c(1,1), seed = 42) {
  set.seed(seed)
  ...
  freq_rown <- dfe_l * budget_l * subject_n
  ...
  data_freq <- data.table(count_s1 = rep(count_s1, times = budget_l),
                          ...
                          subject_n = rep(subject_n, times = freq_rown),
                          beta_n = rep(beta_n, times = freq_rown),
