# ==========================================================================
# Create Stimuli
# Shifting model (Heuristic + RSFT) vs. RSFT
# ==========================================================================
...
rsft_sim <- data.table(
  ...
  hvalue_rsft = predict(rsft_model, type="values")[,1],
  lvalue_rsft = predict(rsft_model, type="values")[,2]
)
...
# Shifting -----------------------------------------------------------------------------------
shifting <- shift_d( ~ prhv_ph + prhv,
        time = ~trial,
        data = sim[nr == 1],
        fix = list(c = 3))

shift_prhv <- predict(shifting)
sim <- cbind(sim, shift_prhv)

# delete colums -----------------------------------------------------------------------------
drops <- c("ev2","x1", "x2", "y1", "y2", "px1", "px2", "py1", "py2", "bmin", "bmax")
sim[, c(drops) := NULL]


fwrite(sim, "../stimuli/stimuli.csv")


# Kriterien --------------------------------------------------------------------------------
# Schwierigkeit
# Badrows
# Vorhersagen: Diff und Dir

sim[ ,cdiff := abs(prhv - prhv_ph)]
sim[ ,cdir := ifelse((prhv >= 0.5 & prhv_ph < 0.5) | (prhv <= 0.5 & prhv_ph > 0.5),1,0)]
sim[ ,c1 := dir*diff ]
sim[ ,prdir := round(sum(dir)/.N,2), by =c("nr", "b")]
sim[ ,prc1 := round(sum(c1)/.N,2), by=c("nr","b")]
sim[, mdiff := mean(diff), by=c("nr","b")]
