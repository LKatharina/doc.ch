...
plotfunc <- function(i){
  ...
  plot <- ggplot(stimuli[nr == unique(nr)[i] & model != "shift"] , 
    ...
    geom_point(agg, mapping = aes(x = trial, y = median, shape = model, colour = model), inherit.aes = F, size = 4, stroke = 2)+
    geom_line(agg, mapping = aes(x = trial, y = median, group = interaction(model), colour = model), linetype = 2, inherit.aes = F, size = 0.8, alpha = 0.5)+
