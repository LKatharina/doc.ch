# ==========================================================================
# Plot: Choice Predictions from a Shifting Model
# Shifting between PH Heuristic (Korn & Bach, 2018) and Optimal Model (Houston & McNamara, 1988)
# ==========================================================================
...
# # Median Shifting + PH & OPT Point Predictions--------------------------------
plotpredictions <- function(i){
  ...
  plot <- ggplot(stimuli[nr == unique(nr)[i] & model != "shift"] , 
    ...
    theme(legend.background = element_rect(linetype = 1, size = 0.01, colour = "transparent", fill = "transparent"),
...
ggsave("../figures/temp_shifting_c.png",width = 5, height = 4, scale = 0.9) # Used in proposal

plotmedian1 <- function(i){
  ...
  plot <- ggplot(agg[ model != "shift"] , 
    ...
    geom_hline(yintercept = 0.5, linetype = 2, size = 0.5, alpha = 0.1)+
    geom_point(size = 4, alpha = 0.5, fill = "grey", show.legend = FALSE) +
    scale_shape_manual(values=c(21,24,3),name = "",labels = c("Probability Heuristic", "Optimal Model", "Shifting Model"))+ # Irgendwie verdreht (????)
    scale_colour_manual(values=c("black","green")) +
    # scale_fill_manual(values=c("grey","grey","blue"), name = "Model",labels = c("Probability Heuristic", "Optimal Model", "Shifting Model (c=3)"))+
    ylim(0,100) +
    scale_y_continuous(breaks = c(0,50,100), limits = c(0,110), expand = c(0,0)) +
    labs(x = 'Choices (1-5)', y = '% Risky Choices')+
    #ggtitle(expression(paste(bold("Figure 2."), italic(" Model Predictions"))))+
    geom_point(agg[model == "shift"], mapping = aes(x = trial, y = median, shape = model), colour = "blue", inherit.aes = F, size = 4, stroke = 2)+
    geom_line(agg[model == "shift"], mapping = aes(x = trial, y = median, group = 1), linetype = 2, inherit.aes = F, colour = "blue", size = 0.8, alpha = 0.5)+
    theme(legend.background = element_rect(linetype = 1, size = 0.01,  colour = "transparent", fill = "transparent"),
...
stimuli[nr == unique(nr)[3]]
s3_opt_ph <- plotmedian1(3)
ggsave("../figures/temp_shifting_sed.png",width = 6, height = 5, scale = 0.91) # used for sed conf
# Shifting -------------------------------------------------------------------------

plotallpreds <- function(i){
  plot <- ggplot(stimuli[nr == unique(nr)[i]], 
    ...
    scale_y_continuous(breaks = seq(0,100, 25), limits = c(0,110), expand = c(0,0)) +
    labs(x = 'Trial', y = '% Risky Choices', fill = "Model")+
