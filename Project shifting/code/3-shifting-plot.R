...
plotfunc <- function(i){
  ...
  agg <- stimuli[nr == unique(nr)[i], .(median = median(prhv)), by = c("model","trial")][model == "shift"]
  agg[,model := as.character(model)]
  ...
  plot <- ggplot(stimuli[nr == unique(nr)[i] & model != "shift"] , 
    ...
    scale_shape_manual(values=c(1,2,3),name = "Model",labels = c("Optimal Model", "Probability Heuristic", "Shifting Model"))+
    ...
    geom_point(agg[model == "shift"], mapping = aes(x = trial, y = median, shape = model), colour = "red", inherit.aes = F, size = 4, stroke = 2)+
    geom_line(agg[model == "shift"], mapping = aes(x = trial, y = median, group = 1), linetype = 2, inherit.aes = F, colour = "red", size = 0.8, alpha = 0.5)+
