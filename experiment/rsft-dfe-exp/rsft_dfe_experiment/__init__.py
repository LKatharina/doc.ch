...
class Consent_eng(Page):
  ...
  def vars_for_template(player):
    return {
      'participation_fee': player.participant.payoff_plus_participation_fee(),
      #'real_world_currency_per_point': cu(1).to_real_world_currency(session),
      #'example_pay': cu(12).to_real_world_currency(session),
      'num_vouchers': 20,
      "duration": C.DURATION
    }
