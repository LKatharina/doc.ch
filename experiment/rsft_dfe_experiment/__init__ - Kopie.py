...
def creating_session(subsession):
  ...
  dfe_conditions = itertools.cycle([True, False]) #geht wiederholt durch die Liste durch
 
  if subsession.round_number == 1: # round_number = eine Variable von subsession
  # In der ersten Runde wird durch die Bedinung geloopt und es wird jedem Spieler eine Bedinung zugeordnet und die Randomisierung der Phasen findet statt
  # Man bestimmt die Anzahl Runden mit der Konstante NUM_ROUNDS , wenn man diese bestimmt hat, dann kann man mit round_number() auf die aktuelle Runde zugreifen. 
    for p in subsession.get_players(): #get_players --> Gibt eine Liste von Spielern in der Subsession zurück
      participant = p.participant
      # for testing set this to true
      participant.dfe_condition = next(dfe_conditions) # Man bestimmt die Bedingungen --> Man ordnet jeder Participant einer Bedingung zu
      participant.num_blocks = 999
      participant.PM = exp.Phasemanager(exp.phases, exp.stimuli, exp.blocks, exp.trials) # Ausführen des exp File --> Damit kann man auf die Varialen des Phasenmanagers zugreifen
      ...
      # Damit kann man auf die Variablen des Appearancemanagers zugreifen

   # in allen Runden
  for p in subsession.get_players():
    participant = p.participant # Participant
    round_number = subsession.round_number 
    phase_number = participant.PM.get_phaseN(round_number) # Phasennr unrandomisiert
    phase = participant.PM.get_phaseL(round_number) # phase label bsp: familiarization (wenn 1 dann)
    stimuli = participant.AM.get_stimuli(round_number, phase_number) # [[3.0, 9.0, 0.6, 0.4], [7.0, 5.0, 0.2, 0.8], [29.0, 0.0]]
    stimulus_position = participant.AM.get_action_position(round_number) # Output [0, 1]
    feature_color = participant.AM.get_feature_appearance(round_number)[0] # Output [[0, 1], [0, 1]]
    # Store variables
    p.phase = participant.PM.get_phaseL(round_number)
    p.block = subsession.round_number
    p.budget = stimuli[2][0]  # [[3.0, 9.0, 0.6, 0.4], [7.0, 5.0, 0.2, 0.8], [29.0, 0.0]] --> Hier 29
    p.stimulus0 = concat_stimulus(0, stimuli) #Name für Stimuli z.B. 3_60_9 (Risky)
    p.stimulus1 = concat_stimulus(1, stimuli) #Name für Stimuli z.B. 7_20_5 (Safe)
    p.state = stimuli[2][1] # [[3.0, 9.0, 0.6, 0.4], [7.0, 5.0, 0.2, 0.8], [29.0, 0.0]] --> Hier 0
    ...
    p.layout_featurecolor = '_'.join(''.join(str(y) for y in x) for x in [feature_color]) #01 oder 01
    p.layout_stimulusposition_01 = ''.join(str(x) for x in stimulus_position) #01 oder 10

    # Initialize variable containers for the blocks
    n = int(C.NUM_ROUNDS + 1) #hier 17
    if (subsession.round_number == 1):
      subsession.session.vars['instruction_rounds'] = participant.PM.get_instruction_rounds() #[2, 6, 7, 11, 12, 16, 17] --> Wieso bis 17?
      participant.bonus_rounds = participant.PM.get_bonus_rounds()
      participant.stimulus_position = [None] * n #[None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None]
    ...
    participant.stimulus_position[round_number] = stimulus_position # Einfüllen des Stimulus
    ...
    participant.img1[round_number] = css_img_orig_position[stimulus_position[0]] # Wenn Stimulus position [0,1] dann Stimulus0 img1, wenn [1,0] dann Stimulus1 auf img1
    participant.img2[round_number] = css_img_orig_position[stimulus_position[1]] # Wenn Stimulus position [0,1] dann Stimulus1 img2, wenn [1,0] dann Stimulus0 auf img2
    if (phase in exp.phases):
      outcomes_orig_position = [draw_outcomes(x, C.NUM_TRIALS) for x in participant.AM.get_stimuli(round_number, phase_number)[ :2]] #Nur Stimuli [[3.0, 9.0, 0.6, 0.4], [7.0, 5.0, 0.2, 0.8]]
      ...
      # Ergibt einen Vektor (verschachtelt) Liste, bei der die erste Ebene die Stimuli angeben und die 2 die gezogenen Outcomes --> z.B.
      participant.outcomes[round_number] = [outcomes_orig_position[i] for i in stimulus_position]
      sampling_outcomes_orig_position = [draw_outcomes(x, C.NUM_SAMPLES_PER_OPTION) for x in participant.AM.get_stimuli(round_number, phase_number)[ :2]] # Ziehen der Outcomes für das Sampling
...
def draw_outcomes(action, size):  #action = [3.0, 9.0, 0.6, 0.4], size = number of samples/trials
  probs = action[2: ][1] # action[2:] --> [0.6,0.4] und davon das Eintrag bei index 1 --> 0.4
  indices = np.random.binomial(n=1, p=probs, size=size) # Ziehung aus einer Binominalverteilung
  #indices = [0, 1, 0] wenn size = 3
  x = action[ :2] #hier wird ein Vektor gebildet, der nur die Outcomes enthält [3,9] 
  res = [x[i] for i in indices] # Aus diesem Vektor wird dann jenachdem, welche Zahl in Indices ist ein Outcome gezogen
  return res
...
class instructions1(Page):
  def is_displayed(player):
    ...
    # Seite wird nur dann angezeigt, wenn der Player in der Familiarisierungphase ist
  form_model = 'player'
